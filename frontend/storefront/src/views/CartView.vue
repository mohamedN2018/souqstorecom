...
<template>
  <div class="container-x py-8">
    ...
    <div v-if="!cart.items.length" class="text-center py-20 text-ink/50">
      السلة فارغة 🛒
      <div class="mt-4">
        <router-link :to="{ name: 'products' }" class="btn-primary inline-block">{{ t("home.shop_now") }}</router-link>
      </div>
    </div>
    ...
    <div v-else class="grid lg:grid-cols-[1fr_320px] gap-6">
      ...
      <aside class="card p-5 h-fit space-y-4 lg:sticky lg:top-20">
        ...
        <button class="btn-primary w-full">إتمام الشراء</button>
